...
public class AddReportModel extends Model {
	...
	public boolean addPendingReport(ReportEntity report,
			Vector<Integer> category, File[] pendingPhotos, String news, String socialmedia) {
		boolean status;
		// add pending reports
		...
		if (status) {
			...
			if (socialmedia != null && socialmedia.length() > 0) {
				...
				MediaEntity media = new MediaEntity();
				media.setMediaId(0);
				media.setLink(socialmedia);
				// get report ID;
				media.setReportId(id);
				media.setType(IMediaSchema.SOCIALMEDIA);
				Database.mMediaDao.addMedia(media);
			}
		}
	...
	public boolean updatePendingReport(int reportId, ReportEntity report,
			...
			String news, String socialmedia) {
		boolean status;
		...
		if (status) {
			...
			if (socialmedia != null && socialmedia.length() > 0) {
				// delete existing news item
				Database.mMediaDao.deleteReportSocialMedia(reportId);
				MediaEntity media = new MediaEntity();
				media.setMediaId(0);
				media.setLink(socialmedia);
				// get report ID;
	...
	public List<MediaEntity> fetchReportSocialMedia(int reportId) {
		return Database.mMediaDao.fetchReportSocialMedia(reportId);
	}
