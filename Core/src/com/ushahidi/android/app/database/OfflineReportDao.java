...
public class OfflineReportDao extends DbContentProvider implements IOfflineReportDao,
    ...
    protected OfflineReport cursorToEntity(Cursor cursor) {
        ...
        if (cursor != null) {
            ...
            if (cursor.getColumnIndex(SOCIALMEDIA) != -1) {
                socialMediaIndex = cursor.getColumnIndexOrThrow(SOCIALMEDIA);
                offlineReport.setSocialMedia(cursor.getString(socialMediaIndex));
            }
            
            if (cursor.getColumnIndex(MENU) != -1) {
                menuIndex = cursor.getColumnIndexOrThrow(MENU);
                offlineReport.setMenu(cursor.getString(menuIndex));
            }
        }
    ...
    private void setContentValue(OfflineReport offlineReport) {
        ...
        initialValues.put(MENU, offlineReport.getMenu());
        initialValues.put(SOCIALMEDIA, offlineReport.getSocialMedia());
    }
