...
public class ViewReportFragment extends SherlockFragment {
	...
	private void initReport(int position) {
		...
		if (mReport != null) {
			...
			socialMedia = mReport.get(position).getIncident().getSocialMedia();
			
			menu = mReport.get(position).getIncident().getMenu();
			
			if((socialMedia != null) && (socialMedia.length() > 0))
				mView.setSocialMedia(mReport.get(position).getIncident().getSocialMedia());
			else
				mView.setSocialMedia("No Social Media");
				
			if((menu!= null) && (menu.length() > 0))
				mView.setMenu(mReport.get(position).getIncident().getMenu());
			else
				mView.setMenu("No Menu");
